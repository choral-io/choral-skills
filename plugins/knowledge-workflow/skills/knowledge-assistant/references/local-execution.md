...
- Resolve the current member using `<knowledge_dir>/.workflow/runtime.md`.
...
- For `run-goal` or `auto-review`, merge execution rules from skill baseline, project-specific rules, and the current prompt. The skill baseline is the hard floor; project and prompt rules may narrow scope or authorize automation only inside that floor.
...
- Keep worktrees under `<worktrees_dir>/`; keep worktree contents out of git.
